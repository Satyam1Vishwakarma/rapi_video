...
pub mod rapi_sync{
    ...
    impl ApiVideo {
        ...
        pub fn video_upload(&self, title: String, full_file_path: String) -> String {
            ...
            if file_len < 209715200 {
                let file = File::open(full_file_path).unwrap();
        ...
        pub fn get_all_video(&self, page_num: u8, page_size: u8) -> serde_json::Value {
            let z = &self.geturl();
            let url = format!(
                "{}/videos?currentPage={}&pageSize={}'",
                z, page_num, page_size
        ...
        pub fn del_video(&self, vid: String) -> StatusCode {
            let url = format!("https://ws.api.video/videos/{}", vid);
            let client = reqwest::blocking::Client::new();
            let res = client
                .delete(url)
